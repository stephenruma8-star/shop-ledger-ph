// Strict-IDB upgrade smoke: simulates a real IndexedDB of an EXISTING install
// (pre-3.4.30, DB version 3, no supplierPayments store) and verifies the current
// bundle boot()s fine and the suppliers route works after the version bump fires
// onupgradeneeded. Real Chromium only runs onupgradeneeded when the requested
// version is HIGHER than the existing one, so any new store added without a
// DB_VERSION bump will make this test fail with NotFoundError.
...
const V3_STORES = ['clients','transactions','payments','inventory','quickItems','settings','auditLogs','users','expenses','suppliers','purchaseOrders','notifications'];
let storeSet = new Set(V3_STORES);
let existingVersion = 3;

function openStrict(name, version) {
  const db = {
    transaction(store, mode) {
      if (!storeSet.has(store)) throw new DOMException('The specified object store was not found.', 'NotFoundError');
      const tx = { objectStore: () => fakeOS, oncomplete: null, onerror: null };
      queueMicrotask(() => { if (tx.oncomplete) tx.oncomplete({}); });
      return tx;
    },
    objectStoreNames: { contains: (s) => storeSet.has(s) },
    createObjectStore: (s) => { storeSet.add(s); return {}; },
  };
  const r = { onupgradeneeded: null, onsuccess: null, onerror: null, result: db };
  queueMicrotask(() => {
    if (version > existingVersion && r.onupgradeneeded) {
      existingVersion = version;
      r.onupgradeneeded({ target: { result: db } });
    }
...
globalThis.self = win;
win.Chart = class { constructor() {} destroy() {} update() {} };
...
Object.defineProperty(globalThis, 'document', { value: {
  getElementById: () => makeEl(), querySelector: () => makeEl(), querySelectorAll: () => [makeEl()],
  createElement: () => makeEl(), createElementNS: () => makeEl(), createTextNode: () => ({}),
...
Object.defineProperty(globalThis, 'indexedDB', { value: { open: openStrict }, configurable: true });
...
try {
  ...
  console.log('STRICT-IDB: boot() completed on existing v3 install');
  if (!storeSet.has('supplierPayments')) throw new Error('supplierPayments store was never created by the upgrade path');
  await Promise.race([
    win.navigate('suppliers'),
    new Promise(res => setTimeout(res, 5000)).then(() => { throw new Error('TIMEOUT navigating suppliers'); })
  ]);
  console.log('STRICT-IDB OK: suppliers route works after DB_VERSION bump');
  process.exit(0);
} catch (e) {
  console.error('STRICT-IDB FAIL:', e.message);
  process.exit(1);
}