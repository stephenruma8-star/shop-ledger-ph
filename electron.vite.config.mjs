...
function copyMainStatic() {
  return {
    ...
    apply: 'build',
    closeBundle() {
      for (const name of ['wsServer.js', 'db.js', 'crypto.js', 'backupService.js', 'lanApi.js', 'logger.js', 'config.js', 'pairing.js']) cpSync(resolve('src/main', name), resolve('out/main', name))
    }
  }
}
