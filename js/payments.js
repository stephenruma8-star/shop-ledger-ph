  ...
  root.innerHTML = `
    <div class="space-y-4 fade-in">
      <div class="flex gap-2 flex-wrap items-center">
        <input id="paySearch" placeholder="Search payments..." class="flex-1 min-w-[200px] px-4 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" oninput="renderPayTable()" />
...
function renderPayTable() {
  ...
  container.innerHTML = `<div class="p-3 bg-gray-50 dark:bg-gray-700 text-sm font-medium flex justify-between"><span>Total Payments: ${sorted.length} entries</span><span class="text-green-600 font-bold">${peso(total)}</span></div>
    ...
    <tbody>${sorted.slice(0, 100).map(p => `<tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
      <td class="p-3 text-gray-500">${fmtDate(p.date)}</td><td class="p-3 font-medium">${p.clientName || 'N/A'}</td>
      <td class="p-3"><span class="px-2 py-0.5 rounded-full text-xs ${p.type === 'Full' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}">${p.type || 'Partial'}</span></td>
      <td class="p-3 text-right font-bold text-green-600">${peso(p.amount)}</td><td class="p-3 text-gray-500 text-xs">${p.notes || ''}</td>
...
function openPaymentModal(id) {
  ...
  modal(`
    <div class="p-6">
      ...
      <div class="space-y-3">
        <div><label class="text-xs text-gray-500 block">Client *</label><select id="pf-client" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800">${clients.map(c => `<option value="${c.id}" ${isEdit && p.clientId === c.id ? 'selected' : ''}>${c.name} (${peso(c.balance)})</option>`).join('')}</select></div>
        ...
        <div><label class="text-xs text-gray-500 block">Notes</label><textarea id="pf-notes" rows="2" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-sm">${isEdit ? (p.notes||'') : ''}</textarea></div>
...
async function savePayment(id) {
  ...
  if (id) {
    ...
    if (oldPay && oldPay.clientId !== clientId) {
