  ...
  root.innerHTML = `
    <div class="space-y-4 fade-in">
      <div class="flex gap-2 flex-wrap items-center">
        <input id="expSearch" placeholder="Search expenses..." class="flex-1 min-w-[200px] px-4 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" oninput="renderExpTable()" />
...
function renderExpTable() {
  ...
  container.innerHTML = `<div class="p-3 bg-gray-50 dark:bg-gray-700 text-sm font-medium flex justify-between"><span>Total Expenses: ${sorted.length} entries</span><span class="text-red-600 font-bold">${peso(total)}</span></div>
    ...
    <tbody>${sorted.slice(0, 100).map(e => `<tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
      <td class="p-3 text-gray-500">${fmtDate(e.date)}</td><td class="p-3">${e.category || '-'}</td><td class="p-3">${e.description || ''}</td>
      <td class="p-3">${e.payee || '-'}</td><td class="p-3 text-right font-bold text-red-600">${peso(e.amount)}</td>
  ...
  modal(`
    <div class="p-6">
      ...
      <div class="space-y-3">
        ...
        <div><label class="text-xs text-gray-500 block">Description</label><input id="ef-desc" value="${isEdit ? (e.description||'') : ''}" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" /></div>
        <div class="grid grid-cols-2 gap-3">
          ...
          <div><label class="text-xs text-gray-500 block">Payee</label><input id="ef-payee" value="${isEdit ? (e.payee||'') : ''}" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" /></div>
        </div>
