  ...
  root.innerHTML = `
    <div class="space-y-4 fade-in">
      <div class="flex gap-2 flex-wrap items-center">
        <input id="supSearch" placeholder="Search suppliers..." class="flex-1 min-w-[200px] px-4 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" oninput="renderSupTable()" />
...
function renderSupTable() {
  ...
  container.innerHTML = `<table class="w-full text-sm"><thead><tr class="bg-gray-50 dark:bg-gray-700 text-left"><th class="p-3">Name</th><th class="p-3">Contact</th><th class="p-3">Email</th><th class="p-3">Category</th><th class="p-3 text-center">Actions</th></tr></thead>
    <tbody>${filtered.map(s => `<tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
      <td class="p-3 font-medium">${s.name}</td><td class="p-3">${s.contact || '-'}</td><td class="p-3 text-gray-500">${s.email || '-'}</td>
...
function openSupplierModal(id) {
  ...
  modal(`
    <div class="p-6">
      ...
      <div class="space-y-3">
        <div><label class="text-xs text-gray-500 block">Name *</label><input id="sf-name" value="${isEdit ? (s.name||'') : ''}" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" /></div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="text-xs text-gray-500 block">Contact</label><input id="sf-contact" value="${isEdit ? (s.contact||'') : ''}" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" /></div>
          <div><label class="text-xs text-gray-500 block">Email</label><input id="sf-email" value="${isEdit ? (s.email||'') : ''}" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" /></div>
        </div>
        <div><label class="text-xs text-gray-500 block">Category</label><input id="sf-category" value="${isEdit ? (s.category||'') : ''}" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" /></div>
        <div><label class="text-xs text-gray-500 block">Address</label><input id="sf-address" value="${isEdit ? (s.address||'') : ''}" class="w-full px-3 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800" /></div>
