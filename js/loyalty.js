async function viewLoyalty(root) {
  await Promise.all([dbLoad('loyaltyPoints'), dbLoad('clients')]);
  ...
  root.innerHTML = `
    <div class="space-y-4 fade-in">
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border-l-4 stat-card border-yellow-500">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm glass-card">
          ...
          ${topEarners.length === 0 ? '<p class="text-gray-400 text-sm">No data yet</p>' : topEarners.map((l, i) => `
            <div class="flex justify-between py-2 border-b dark:border-gray-700 last:border-0">
              <span><span class="text-gray-400 mr-2">#${i+1}</span>${escapeHtml(l.clientName || 'Unknown')}</span><span class="font-semibold text-yellow-600">${l.points} pts</span>
            </div>`).join('')}
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm glass-card">
          ...
          ${activity.length === 0 ? '<p class="text-gray-400 text-sm">No activity</p>' : activity.slice(0, 20).map(l => `
            <div class="flex justify-between py-1 border-b dark:border-gray-700 last:border-0 text-sm">
              <span>${escapeHtml(l.clientName || 'Unknown')}</span><div><span class="text-yellow-600 font-medium">+${l.points} pts</span><span class="text-gray-400 ml-2 text-xs">${fmtDateTime(l.updatedAt)}</span></div>
            </div>`).join('')}
        </div>
      </div>
    </div>`;
