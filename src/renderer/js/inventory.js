...
export function renderInvTable() {
  ...
  if (!container) return;
  if (sorted.length === 0) { container.innerHTML = '<div class="text-center py-12 text-gray-400"><svg class="mx-auto mb-3" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg><p class="font-medium">No inventory items</p><p class="text-sm mt-1">Add products to track stock levels</p></div>'; return; }
  container.innerHTML = `<table class="data-table"><thead><tr><th class="p-3 w-10"><input type="checkbox" onchange="document.querySelectorAll('.inv-check').forEach(c=>c.checked=this.checked);toggleInvBulkBar()" /></th><th class="p-3 w-12">Photo</th><th class="p-3">Name</th><th class="p-3">SKU</th><th class="p-3">Category</th><th class="p-3 text-right">Price</th>        <th class="p-3 text-right">Cost</th><th class="p-3 text-center">Stock</th><th class="p-3 text-left">Unit</th><th class="p-3">Expiry</th><th class="p-3 text-center">Actions</th></tr></thead>
    <tbody>${sorted.map(i => {
...
export let debouncedRenderInvTable = debounce(renderInvTable, 300);
...
export function filterExpiringSoon() {
  ...
  const container = document.getElementById('invTable');
  if (!container) return;
  container.innerHTML = `<table class="data-table"><thead><tr><th class="p-3">Photo</th><th class="p-3">Name</th><th class="p-3">SKU</th><th class="p-3 text-center">Stock</th><th class="p-3">Expiry</th><th class="p-3">Status</th></tr></thead>
