...
export let debouncedRenderTxTable = debounce(renderTxTable, 300);
...
export function renderTxTable() {
  ...
  if (!container) return;
  if (sorted.length === 0) { container.innerHTML = '<div class="text-center py-12 text-gray-400"><svg class="mx-auto mb-3" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg><p class="font-medium">No transactions yet</p><p class="text-sm mt-1">Sales will appear here once you create them</p></div>'; return; }
  container.innerHTML = `<table class="data-table"><thead><tr><th class="p-3 w-10"><input type="checkbox" onchange="document.querySelectorAll('.tx-check').forEach(c=>c.checked=this.checked);toggleTxBulkBar()" /></th><th class="p-3">Invoice</th><th class="p-3">Date</th><th class="p-3">Client</th><th class="p-3">Items</th><th class="p-3 text-right">Total</th><th class="p-3">Method</th><th class="p-3">Status</th><th class="p-3 text-center">Actions</th></tr></thead>
    <tbody>${items.map(t => {
      const isVoided = t.status === 'voided';
      const isReturn = t.status === 'return';
      const isInterest = t.status === 'interest';
  const statusBadge = isVoided ? '<span class="badge badge-voided font-semibold">VOIDED</span>' : isReturn ? '<span class="badge badge-returned font-semibold">RETURN</span>' : isInterest ? '<span class="badge badge-pending font-semibold">INTEREST</span>' : t.status === 'pending' ? '<span class="badge badge-pending">Pending</span>' : '<span class="badge badge-paid">Paid</span>';
  ...
  modal(`
    <div class="p-6">
      ...
      <div class="flex justify-end gap-2">
        ...
        <button onclick="confirmVoidTransaction(${id})" class="btn btn-danger">Void Transaction</button>
      </div>
    </div>
  `);
}
